...
class compile_sym(Task):
    def run(self):
        lsyms = [line.strip() for line in self.inputs[0].read().split()]
        lsyms.sort()
...
def do_the_symbol_stuff(self):
    tsk = self.create_task(
        ...
        self.path.find_or_declare(
            getattr(self, 'sym_filename', f'{self.target}.def')
        ),
    )
    ...
    if 'msvc' in (self.env.CC_NAME, self.env.CXX_NAME):
        self.link_task.env.append_value(
            'LINKFLAGS', [f'/def:{tsk.outputs[0].bldpath()}']
        )
        ...
        self.link_task.env.append_value(
            'LINKFLAGS',
            ['-Wl,-version-script', f'-Wl,{tsk.outputs[0].bldpath()}'],
        )
